...
3. Backend (docker):
   ...
   - docker run -e DB_* -e JWT_SECRET -p 3000:3000 sports-analytics-backend:latest
4. Frontend (local):
...
Notas finales y recomendaciones para Imagine Cup
------------------------------------------------
- Fortalezas a destacar en la presentación: arquitectura modular, uso de Go para backend de alto rendimiento, worker interno para auto-liquidación, y diseño frontend moderno con UX enfocada en métricas y control de riesgo.
- Puntos de mejora/roadmap para mostrar innovación:
  - Reemplazar la lógica heurística actual de AI Tips por un pipeline ML/LLM (Azure OpenAI + entrenamiento con datos anonimizados) y mostrar un prototipo offline de "predicción de EV" para una liga concreta.
  - Separar el worker y añadir cola de trabajo para robustez y observabilidad.
  - Implementar tests end-to-end y migraciones versionadas para cumplir con prácticas empresariales.

Si se desea, se puede generar un diagrama de arquitectura (SVG/PNG), un runbook detallado de despliegue en Azure Container Apps, y una sección adicional con métricas de negocio esperadas para el pitch de Imagine Cup (KPI y proyecciones).